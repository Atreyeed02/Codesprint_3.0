# CodeSprint 3.0 🚀

> *45-day coding challenge organized by USC KIIT — a consistency-driven journey across Data Structures, Algorithms, and Competitive Programming.*

![Challenge](https://img.shields.io/badge/Challenge-CodeSprint%203.0-1D9E75?style=flat-square)
![Status](https://img.shields.io/badge/Status-Completed%20✅-1D9E75?style=flat-square)
![Duration](https://img.shields.io/badge/Duration-45%20Days-0F6E56?style=flat-square)
![Problems Solved](https://img.shields.io/badge/Problems%20Solved-90-854F0B?style=flat-square)
![Languages](https://img.shields.io/badge/Languages-C%2B%2B%20%7C%20Java-3C3489?style=flat-square)

---

## 📖 Table of Contents

- [About the Challenge](#-about-the-challenge)
- [About Me](#-about-me)
- [Key Highlights](#-key-highlights)
- [Progress Tracker](#-progress-tracker)
- [Repository Structure](#-repository-structure)
- [Topics Covered](#-topics-covered)
- [Tech Stack](#️-tech-stack)
- [What This Repository Demonstrates](#-what-this-repository-demonstrates)
- [Connect With Me](#-connect-with-me)

---

## 📌 About the Challenge

| Field | Details |
|-------|---------|
| 🗓️ Duration | 45 Days |
| 🏫 Organizer | USC KIIT |
| 💻 Languages | C++, Java |
| 📈 Focus | DSA · Problem Solving · Competitive Programming · Consistency |

---

## 👩‍💻 About Me

Hi, I'm **Atreyee Debnath** — a Computer Science student passionate about building things that matter.

**Interests:**
- 🧠 DSA & Competitive Programming
- ⚙️ Backend Development
- 🤖 AI/ML Integration
- 🛠️ Building Real-World Projects

---

## ✨ Key Highlights

- 🔥 **100% consistency** — all 45 days completed back-to-back, no days skipped
- 🧩 **90 problems solved**, roughly 4 in C++ (Days 1–2) and 86 in Java (Days 3–45)
- 📈 **Clear progression** — from array basics and pattern printing (Day 1) to Floyd-Warshall and bipartite graphs (Day 45)
- 🗂️ **25+ distinct DSA topics** covered, spanning linear data structures, trees, graphs, DP, and advanced structures like Fenwick Trees
- 📝 **Themed problem statements** — many problems are framed as named scenarios (e.g. *"Prateek's Diagonal Energy Grid"*, *"The Josephus Problem"*) rather than generic prompts

---

## 📊 Progress Tracker

**45 / 45 days completed · 90 problems solved**

`████████████████████████████████████████████` **100%**

<details>
<summary>📅 View full day-by-day tracker</summary>

```
Day  1  ✅  |  Day 16  ✅  |  Day 31  ✅
Day  2  ✅  |  Day 17  ✅  |  Day 32  ✅
Day  3  ✅  |  Day 18  ✅  |  Day 33  ✅
Day  4  ✅  |  Day 19  ✅  |  Day 34  ✅
Day  5  ✅  |  Day 20  ✅  |  Day 35  ✅
Day  6  ✅  |  Day 21  ✅  |  Day 36  ✅
Day  7  ✅  |  Day 22  ✅  |  Day 37  ✅
Day  8  ✅  |  Day 23  ✅  |  Day 38  ✅
Day  9  ✅  |  Day 24  ✅  |  Day 39  ✅
Day 10  ✅  |  Day 25  ✅  |  Day 40  ✅
Day 11  ✅  |  Day 26  ✅  |  Day 41  ✅
Day 12  ✅  |  Day 27  ✅  |  Day 42  ✅
Day 13  ✅  |  Day 28  ✅  |  Day 43  ✅
Day 14  ✅  |  Day 29  ✅  |  Day 44  ✅
Day 15  ✅  |  Day 30  ✅  |  Day 45  ✅
```

</details>

---

## 📂 Repository Structure

<details>
<summary>🗂️ Click to expand full folder tree (Day-01 → Day-45)</summary>

```
CodeSprint_3.0/
│
├── Day-01/
│   ├── move_zeroes.cpp               # Array manipulation — in-place zero shifting
│   └── contains_duplicate.cpp        # Hash set duplicate detection
│
├── Day-02/
│   ├── nitin_average_score.cpp       # Arithmetic mean computation
│   └── soumya_audit_report.cpp       # Conditional report generation
│
├── Day-03/
│   ├── alarm_code_validator.java     # String validation logic
│   └── alphabet_cross_pattern.java   # 2D pattern printing
│
├── Day-04/
│   ├── firewall_pattern.java         # Nested loop pattern design
│   └── announcement_analyzer.java    # String frequency analysis
│
├── Day-05/
│   ├── gossip_free_library.java      # HashMap-based filtering
│   └── attendance_tracker.java       # Frequency map & conditionals
│
├── Day-06/
│   ├── check.java                    # Conditional logic
│   └── prateeks_diagonal_energy_grid.java  # Matrix diagonal traversal
│
├── Day-07/
│   ├── soumyadip_and_the_mirror_portal.java  # String reversal
│   └── marutwan_and_the_signal_decoder.java  # Character decoding
│
├── Day-08/
│   ├── rahul_and_the_silent_classroom_challenge.java  # Sorting simulation
│   └── prema_and_the_fest_bracelet_scanner.java       # Linear scan & matching
│
├── Day-09/
│   ├── abhisheks_sorted_squares_challenge.java             # Two-pointer squares
│   └── sagnik_and_the_netflix_watch_history_analyzer.java  # Sliding window / history
│
├── Day-10/
│   ├── poulam_and_the_ancient_treasure_lock.java                       # HashMap locking logic
│   └── nishita_and_the_quickbite_restaurant_recommendation_engine.java # Greedy/priority selection
│
├── Day-11/
│   ├── namita_and_the_stable_rankings.java       # Stable sort implementation
│   └── sitanshu_and_the_college_club_lineup.java # Queue/ordering problem
│
├── Day-12/
│   ├── longest_palindromic_substring.java  # Expand-around-center / DP
│   └── absolute_sum.java                   # Array absolute value aggregation
│
├── Day-13/
│   ├── reverse_words_in_a_string.java  # Two-pointer / stack approach
│   └── insertion_sort_shift_count.java # Modified insertion sort
│
├── Day-14/
│   ├── selection_sort_swap_count.java  # Selection Sort + swap counting
│   └── maximum_visibility_score.java   # Two Pointer approach
│
├── Day-15/
│   ├── username_leaderboard_ranking.java  # Custom Comparator sorting
│   └── minimum_boat_load.java             # Greedy + Two Pointer approach
│
├── Day-16/
│   ├── jewels_and_stones.java  # HashSet character matching
│   └── burnout_zone.java       # Sliding Window technique
│
├── Day-17/
│   ├── reverse_linked_list.java       # Reverse a linked list
│   └── remove_kth_node_from_end.java  # Two Pointer linked list problem
│
├── Day-18/
│   ├── detect_loop_in_linked_list.java             # Floyd's Cycle Detection (Tortoise & Hare)
│   └── intersection_point_of_two_linked_lists.java # Find intersection using Two Pointers
│
├── Day-19/
│   ├── josephus_problem.java           # Last remaining participant in circular linked list
│   └── rotate_doubly_linked_list.java  # Rotate doubly linked list by K positions
│
├── Day-20/
│   ├── LRUCache.java               # LRU cache implementation with O(1) GET and PUT operations
│   └── XORLinkedListTraversal.java # Print all nodes of XOR linked list from left to right
│
├── Day-21/
│   ├── FlattenSortedLinkedLists.java # Merge K sorted linked lists using Min Heap
│   └── NthFibonacciRecursion.java    # Find N-th Fibonacci number using recursion
│
├── Day-22/
│   ├── rat_in_a_maze.java                 # Recursion + Backtracking to find all valid paths
│   └── merge_sort_divide_and_conquer.java # Merge Sort using Divide & Conquer approach
│
├── Day-23/
│   ├── temperature_range_difference.java # Range Queries (Max - Min) using Brute Force / Segment Tree concept
│   └── range_score_frequency_square.java # HashMap Frequency Counting + Range Score Calculation
│
├── Day-24/
│   ├── maximum_consecutive_tokens.java # Sliding Window (Longest Subarray with Sum ≤ W)
│   └── middle_node_linked_list.java    # Fast & Slow Pointer (Second Middle Node)
│
├── Day-25/
│   ├── maximum_banner_display_area.java # Two Pointers (Container With Most Water)
│   └── minimum_window_substring.java    # Sliding Window + HashMap (Minimum Window Substring)
│
├── Day-26/
│   ├── clone_linked_list_random_pointer.java # HashMap (Clone Linked List with Random Pointer)
│   └── tower_of_hanoi.java                   # Recursion (Tower of Hanoi)
│
├── Day-27/
│   ├── peak_transmission_bitonic_array.java # Binary Search (Bitonic Array Peak)
│   └── treasure_vault_search_matrix.java    # Binary Search (Search in Sorted Matrix)
│
├── Day-28/
│   ├── placement_interview_queue.java # Queue Operations (REGISTER / WITHDRAW / NEXT)
│   └── attendance_log_validation.java # Stack (Valid Parentheses with () [] {} <>)
│
├── Day-29/
│   ├── expression_evaluator.java       # Stack-based calculator with operator precedence (+, -, *, /)
│   └── remove_adjacent_duplicates.java # Stack-based adjacent duplicate character removal
│
├── Day-30/
│   ├── prefix_expression_evaluation.java # Evaluate prefix expressions using Stack
│   └── infix_to_postfix_conversion.java  # Convert infix expressions to postfix using Stack
│
├── Day-31/
│   ├── minimum_stress_stack.java # Min Stack with O(1) minimum retrieval
│   └── lift_request_queue.java   # Queue implementation using two stacks
│
├── Day-32/
│   ├── registration_token_system.java     # Queue-based registration/token handling
│   └── parcel_counter_circular_queue.java # Circular Queue using array and modulo indexing
│
├── Day-33/
│   ├── crowd_noise_sliding_window.java # Sliding Window Maximum using Deque
│   └── airport_boarding_priority.java  # Priority Queue with custom comparator
│
├── Day-34/
│   ├── rotting_oranges_multi_source_bfs.java   # Multi-Source BFS
│   └── maximum_energy_drinks_unbounded_dp.java # Unbounded Knapsack DP
│
├── Day-35/
│   ├── scholarship_eligibility_prefix_sum.java  # Sorting + Prefix Sum
│   └── balanced_package_pairs_two_pointers.java # Two Pointers + Sorting
│
├── Day-36/
│   ├── unique_number_position.java # Find unique element using XOR and its position in sorted array
│   └── scholarship_rank_query.java # Rank lookup after sorting marks in descending order
│
├── Day-37/
│   ├── top_k_highest_priority_requests.java # Find K largest elements using Min Heap (Priority Queue)
│   └── count_subarrays_sum_k.java           # Count subarrays with sum K using Prefix Sum + HashMap
│
├── Day-38/
│   ├── letter_combinations_phone_number.java # Generate all keypad letter combinations using Backtracking
│   └── longest_consecutive_sequence.java     # Find longest consecutive sequence using HashSet in O(N)
│
├── Day-39/
│   ├── n_queens_backtracking.java    # Place N queens without attacking each other
│   └── word_search_backtracking.java # Find a word in a 2D grid using DFS + Backtracking
│
├── Day-40/
│   ├── even_level_treasure_tree.java # Find the total treasure present at even levels of a rooted tree using BFS
│   └── kth_largest_bst.java          # Find the K-th largest element in a BST using Reverse Inorder Traversal
│
├── Day-41/
│   ├── diagonal_zigzag_traversal.java # Print the matrix in zigzag diagonal order
│   └── boundary_traversal_tree.java   # Print the anti-clockwise boundary traversal of a binary tree
│
├── Day-42/
│   ├── recover_bst_morris_traversal.java # Recover a Binary Search Tree using Morris Traversal
│   └── subtree_sum_queries.java          # Euler Tour + Fenwick Tree for subtree sum queries
│
├── Day-43/
│   ├── earliest_arrival_with_expiring_roads.java # Modified Dijkstra's Algorithm with Edge Expiry
│   └── infinite_profit_detection.java            # Modified Bellman-Ford for Maximum Profit & Positive Cycle Detection
│
├── Day-44/
│   ├── euler_path.java           # Euler Path in Directed Graph (Ribbon Assembly)
│   └── hamiltonian_path_dfs.java # Hamiltonian Path using DFS + Backtracking (Vaccination Route)
│
├── Day-45/
│   ├── floyd_warshall.java  # All-Pairs Shortest Path (Research Station Wormholes)
│   └── bipartite_graph.java # Bipartite Graph using BFS (Task Pipeline Assignment)
│
└── README.md
```

</details>

> 💡 A quick note: Day-31's `lift_request_queue.java` and Day-32's `registration_token_system.java` both carried the same comment in the original README ("Queue implementation using two stacks"). I've reworded Day-32's to avoid the duplicate — worth double-checking against the actual code to confirm which description is accurate.

---

## 🧠 Topics Covered

| Category | Topic | Days |
|---|---|---|
| **Arrays & Strings** | Array Manipulation | 1, 9, 23, 24 |
| | String Processing | 3, 4, 7, 12, 13, 25 |
| | Pattern Printing | 3, 4, 6 |
| | Matrix / 2D Arrays | 6, 27, 39, 41 |
| **Hashing** | Hash Maps & Sets | 1, 5, 10, 16, 23, 25, 26, 37 |
| **Searching & Sorting** | Sorting Algorithms | 8, 11, 13, 14, 15, 22, 35 |
| | Binary Search | 27, 36 |
| | Divide & Conquer | 22 |
| **Two Pointers / Windows** | Sliding Window | 9, 16, 24, 25, 33 |
| **Greedy** | Greedy Algorithms | 10, 15 |
| **Recursion & Backtracking** | Recursion | 19, 21, 22, 26 |
| | Backtracking | 22, 38, 39 |
| **Linear Structures** | Linked List | 17, 18, 19, 20, 21, 24, 26 |
| | Stack | 28, 29, 30, 31 |
| | Queue | 11, 28, 31, 32 |
| | Caching / System Design | 20 |
| **Heaps** | Priority Queue / Heap | 21, 33, 37, 43 |
| **Trees** | Trees (Binary Tree / BST) | 40, 41, 42 |
| | Tree Traversals | 40, 41, 42 |
| **Advanced Structures** | Segment Tree Concepts | 23 |
| | Fenwick Tree (BIT) | 42 |
| | Range Queries | 23, 42 |
| **Dynamic Programming** | Dynamic Programming | 12, 34 |
| **Graphs** | Graph Algorithms | 34, 40, 43, 45 |
| | Breadth-First Search (BFS) | 34, 40, 43, 45 |
| | Shortest Path Algorithms | 43, 45 |
| | Dijkstra's Algorithm | 43 |
| | Bellman-Ford Algorithm | 43 |
| | Floyd-Warshall Algorithm | 45 |
| | All-Pairs Shortest Path | 45 |
| | Bipartite Graph | 45 |

---

## 🛠️ Tech Stack

![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)

---

## 🚀 What This Repository Demonstrates

- ✅ **Consistency** — solving problems every single day without breaks
- ✅ **Breadth** — covering arrays, strings, sorting, DP, hashing, matrices, trees, and graphs
- ✅ **Multi-language proficiency** — C++ for competitive speed, Java for OOP problems
- ✅ **Clean code** — readable filenames, organized folder structure, commented solutions
- ✅ **Problem-solving mindset** — tackling real-world themed challenges across domains

---

## 🔗 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Atreyee%20Debnath-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/atreyee-debnath/)
[![GitHub](https://img.shields.io/badge/GitHub-Atreyeed02-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Atreyeed02)

---

*Built with consistency, one day at a time · CodeSprint 3.0 × USC KIIT*
